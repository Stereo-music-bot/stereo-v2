...
export default class SoundcloudCommand extends BaseCommand {
  constructor() {
    super('soundcloud', {
      category: 'Music', 
      aliases: [],
      description: 'Search for a soundcloud song to play.',
      usage: '<song name>',
      ownerOnly: false,
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    try {
      result = await rest.search(
        song.includes('https://') 
          ? encodeURI(song)
          : `scsearch:${encodeURIComponent(song)}`
      );
    } catch (e) {
      ...
      return client.Webhook.send(`> ❌ | New error | **${message.guild.name}** | SC Play Command Error | Error: \`${e}\``);
    }
